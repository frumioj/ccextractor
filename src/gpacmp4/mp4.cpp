...
static int process_avc_sample(u32 timescale, GF_AVCConfig* c, GF_ISOSample* s)
{
	int status = 0;
	u32 i;
	const LLONG timestamp = (LLONG )((s->DTS + s->CTS_Offset) * 1000) / timescale;

	current_pts=(LLONG )(s->DTS + s->CTS_Offset)*MPEG_CLOCK_FREQ/timescale ; // Convert frequency to official one
    if (pts_set==0)
		pts_set=1;
	set_fts();
