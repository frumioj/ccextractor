...
{
    dbg_print(DMT_VERBOSE, "user_data(%d)\n", udtype);
    ...
    if ( !memcmp(ud_header,"\x43\x43", 2 ) )
    {
        ...
        dbg_print(DMT_VERBOSE, "Reading %d%s DVD CC segments\n",
        ...
        for (int i=0; i<capcount; i++)
        {
            for (int j=0;j<2;j++)
            {
                ...
                {
                    dbg_print(DMT_VERBOSE, "Illegal caption segment - stop here.\n");
                    maybeextracb = 0;
                    break;
                }
            }
        ...
        while ( maybeextracb && (next_u8(ustream)&0xFE) == 0xFE )
        {
            for (int j=0;j<2;j++)
            {
                ...
                {
                    dbg_print(DMT_VERBOSE, "Illegal (extra) caption segment - stop here.\n");
                    maybeextracb = 0;
                    break;
                }
            }
        ...
        dbg_print(DMT_VERBOSE, "Read %d/%d DVD CC blocks\n", rcbcount, ecbcount);
    }
    // SCTE 20 user data
    else if (ud_header[0] == 0x03)
    {
        if ((ud_header[1]&0x7F) == 0x01)
        {
            ...
            dbg_print(DMT_VERBOSE, "Reading %d SCTE 20 CC blocks\n", cc_count);
            ...
            dbg_print(DMT_VERBOSE, "Reading SCTE 20 CC blocks - done\n");
        }
    ...
    else if ( !memcmp(ud_header,"\x47\x41\x39\x34", 4 ) )
    {
        ...
        if (type_code==0x03) // CC data.
        {
            ...
            if (process_cc_data)
            {
                dbg_print(DMT_VERBOSE, "Reading %d HDTV CC blocks\n", cc_count);
                ...
                if (!proceed)
                {
                    dbg_print(DMT_VERBOSE, "\rThe following payload is not properly terminated.\n");
                    dump (DMT_VERBOSE, cc_data, cc_count*3+1, 0, 0);
                }
                dbg_print(DMT_VERBOSE, "Reading %d HD CC blocks\n", cc_count);
                ...
                dbg_print(DMT_VERBOSE, "Reading HDTV blocks - done\n");
            }
        }
    ...
    else if ( !memcmp(ud_header,"\x05\x02", 2 ) )
    {
        ...
        dbg_print(DMT_VERBOSE, "Reading Dish Network user data\n");
        ...
        dbg_print(DMT_PARSE, "DN  ID: %02X  Count: %5u  Unknown: %04X  Pattern: %X",
        ...
        case 0x02:
            ...
            dbg_print(DMT_PARSE, "%s", debug_608toASC( dishdata, 0) );
        ...
        case 0x05:
            ...
            // 2-3: prev dcd[3-4] 
            // 4-5: prev dcd[5-6]
            dbg_print(DMT_PARSE, " - %02X  pch: %02X %5u %02X:%02X\n",
                       dcd[0], dcd[1],
            ...
            dbg_print(DMT_PARSE, "%02X %02X  %02X:%02X",
                       dcd[0], dcd[1], dcd[2], dcd[3]);
            ...
            if (type==0x02)
            {
                ...
                dbg_print(DMT_PARSE, " - R:%02X :%s", type, debug_608toASC( dishdata, 0) );
        ...
        dbg_print(DMT_VERBOSE, "Reading Dish Network user data - done\n");
    }
    ...
    else
    {
        ...
        dbg_print(DMT_VERBOSE, "Unrecognized user data:\n");
        int udatalen = ustream->end - ustream->pos;
        dump (DMT_VERBOSE, ustream->pos, (udatalen > 128 ? 128 : udatalen),0 ,0);
    }

    dbg_print(DMT_VERBOSE, "User data - processed\n");
